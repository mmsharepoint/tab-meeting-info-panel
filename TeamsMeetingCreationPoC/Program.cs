...
string meetingSubject = "Test Meeting with App/Tab 4";
...
//var newOM = await graphClient
//        .Users[userID].OnlineMeetings.PostAsync(om);
//Console.WriteLine($"Online Meeting created with ID {newOM.Id}");
//Console.WriteLine($"Event created with JoinUrl {newOM.JoinWebUrl}");
//string joinUrl = newOM.JoinWebUrl;
...
Console.WriteLine(azrCtrl.GetConfigurationValue($"TEAMSMEETINGSERVICECALL:{chatId}:CUSTOMERNAME") ?? "No customername");
Console.WriteLine(azrCtrl.GetConfigurationValue($"TEAMSMEETINGSERVICECALL:{chatId}:CUSTOMERPHONE") ?? "No customerphone");
Console.WriteLine(azrCtrl.GetConfigurationValue($"TEAMSMEETINGSERVICECALL:{chatId}:CUSTOMEREMAIL") ?? "No customeremail");
Console.WriteLine(azrCtrl.GetConfigurationValue($"TEAMSMEETINGSERVICECALL:{chatId}:CUSTOMERID") ?? "No customerid");
Console.ReadLine();